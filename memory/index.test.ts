...
describe("memory plugin e2e", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(tmpdir(), "openclaw-memory-test-"));
  ...
  test("memory plugin registers and initializes correctly", async () => {
    ...
    expect(memoryPlugin.id).toBe("memory-lancedb-claw");
...
describeLive("memory plugin live tests", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(tmpdir(), "openclaw-memory-live-"));
    dbPath = path.join(tmpDir, "lancedb");
  });
